...
void BotCommands::watch_paste_key(TgBot::Bot& bot,
    ...
    if (password == "") {
        watch_paste(bot, all_keyboards, message, message->text, private_key, old_message_id);
        bot.getApi().deleteMessage(message->chat->id, message->messageId);
    } else {
        ...
        SqlRelation::changeUserState(message->chat->id, conditions::watch_paste_password, message->text, new_message_id);
    }
}
...
void BotCommands::watch_paste_password(TgBot::Bot& bot,
    ...
    if (message->text != password) {
        ...
        watch_paste(bot, all_keyboards, message, workPaste, private_key, old_message_id);
        bot.getApi().deleteMessage(message->chat->id, message->messageId);
