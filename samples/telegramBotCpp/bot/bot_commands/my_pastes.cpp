...
std::vector<std::pair<std::string, std::string>> BotCommands::make_paste_buttons(TgBot::Bot& bot, 
    ...
    std::vector<std::vector<std::string> > last_pastes = gRPC_connect::getLastPastes(message->chat->id, 9);
    ...
    for (size_t i = 0; i < 8; ++i) {
        if (inx >= last_pastes.size()) {
            ...
            buttons.push_back({short_name(last_pastes[inx][0], 15), last_pastes[inx][2]});
        }
        ++inx;
    }
...
void BotCommands::send_paste_settings(TgBot::Bot& bot, 
    ...
    RedisRelation::changeUserState(user_id, conditions::basic, workPaste, new_message_id);
...
void BotCommands::validate_paste_old_password(TgBot::Bot& bot, 
    ...
    if (password != message->text) {
        ...
        RedisRelation::changeUserState(message->chat->id, conditions::change_paste_password, workPaste, new_message_id);
...
void BotCommands::send_my_pastes_list(TgBot::Bot& bot, 
    ...
    std::vector<std::vector<std::string> > last_pastes = gRPC_connect::getLastPastes(message->chat->id, 250);
